...
export function ContentEntry({ id, title, date, description, tags }: ContentEntryProps) {
  return (
    <article className="group p-margin-lg flex flex-col gap-margin-sm hover:bg-surface-container transition-all cursor-pointer relative overflow-hidden">
      <div className="absolute top-0 right-0 p-unit font-label-sm text-label-sm text-secondary-container opacity-50">
        {id}
      </div>
      <div className="flex justify-between items-start">
        <h2 className="font-headline-md text-headline-md text-primary group-hover:text-primary-container transition-colors uppercase">
          {title}
        </h2>
